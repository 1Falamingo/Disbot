...
//############################################################################################
    try {
      ...
      if(message.content.endsWith(config.token)) {
          return;
      } else {
...
//############################################################################################
    client.on('message', message => {
      if(message.content.endsWith(config.token)) message.react(emojinotok);
      ...
      if(message.content.includes(config.BOT_ID)) message.channel.send("<:476351684138893312:498454490467270668>");
...
//############################################################################################
    client.on('guildMemberAdd', member => {
      ...
      .addField("**Пропиши: `${config.token}rules` чтобы узнать правила.**", "Если есть вопросы - пиши модераторам или администраторам данного сервера")
      member.send({ embed });
    });
//############################################################################################
   client.on('message', message => {
